...
os.environ.setdefault('WANDB_ENTITY', 'rawan_khalifa-minerva-university')
...
def train(env_name, sb3_algo, workers, path_to_model=None, wandb_entity=None, disable_wandb=False):
	config = {
	...
	# Initialize wandb with optional entity specification
	if not disable_wandb:
		...
		import time
		run = None
		save_path = f"{model_dir}/training_{int(time.time())}"
	...
	# Setup callbacks - conditionally include WandbCallback
	callbacks = [eval_callback, cust_callback]
	if not disable_wandb and run:
		callbacks.insert(0, WandbCallback(
...
if __name__ == '__main__':
	...
	parser.add_argument('-e', '--entity', help='Wandb entity/organization name', default='rawan_khalifa-minerva-university')
	...
	if args.train:
		if args.file is None:
			...
			train(args.gymenv, args.sb3_algo, args.workers, wandb_entity=args.entity, disable_wandb=args.no_wandb)
		elif os.path.isfile(args.file):
			print(f'<< Continuing {args.file} >>')
			train(args.gymenv, args.sb3_algo, args.workers, args.file, wandb_entity=args.entity, disable_wandb=args.no_wandb)
